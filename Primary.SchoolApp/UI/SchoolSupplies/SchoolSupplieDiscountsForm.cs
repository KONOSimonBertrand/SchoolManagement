...
{
    internal class SchoolSupplieDiscountsForm : SchoolManagement.UI.StudentItemsForm
    {
        private readonly ISchoolSupplieService schoolSupplieService;
        private readonly ILogService logService;
        private readonly ClientApp clientApp;
        private StudentEnrolling selectedEnrolling;
        public SchoolSupplieDiscountsForm(ISchoolSupplieService schoolSupplieService, ClientApp clientApp, ILogService logService)
        {
        ...
        {
            AppUtilities.PrintGridView(DataGridView, Language.TitleDiscountList);
        }
        ...
        private void SaveButton_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowAddSchoolSupplieDiscountForm();
            }
            else
            {
        ...
        {
            var discountList = schoolSupplieService.GetSchoolSupplieDiscountByEnrollingList(enrollingId).Result;
            DataGridView.DataSource = discountList;
        ...
        private void CreateGridViewColumn()
        {
            ...
            GridViewCheckBoxColumn stateColumn = new("IsActive");
            ...
            stateColumn.HeaderText = Language.labelStatus;
            ...
            {
                new GridViewSummaryItem("Discount", " {0}", GridAggregateFunction.Sum)
            };
        ...
        private void DataGridView_ContextMenuOpening(object sender, ContextMenuOpeningEventArgs e)
        {
            ...
            {
                if (DataGridView.CurrentRow.DataBoundItem is SchoolSupplieDiscount record)
                {
                    if (record.IsActive)
                    {
                    ...
                    var statusMenuLabel = record.IsActive ? Language.LabelDesactivateDiscount : Language.LabelActivateDiscount;
                    RadMenuItem changeStatusMenu = new(statusMenuLabel);
                   
                    e.ContextMenu.Items.Add(changeStatusMenu);
                    changeStatusMenu.Click += ChangeStatusMenu_Click;
                }
            }
        }

        private void ChangeStatusMenu_Click(object sender, EventArgs e)
        {
            if (!Program.CurrentSchoolYear.IsClosed)
            {
                if (DataGridView.CurrentRow.DataBoundItem is SchoolSupplieDiscount record)
                {
                    ...
                    if (schoolSupplieService.ChangeStateSupplieDiscount(record).Result)
                    {
                        //enregistrement du log
                        var message = record.IsActive ? $"Désactivation de la réduction du {record.Date}" : $"Activation de la réduction du {record.Date}";
                        Log log = new()
                        {
                            UserAction = $"{message} pour {record.CashFlowType.Name} de l'élève {selectedEnrolling.Student.FullName}  par l'utilisateur {clientApp.UserConnected.UserName} sur le poste {clientApp.IpAddress}",
        ...
        private void EditMenu_Click(object sender, EventArgs e)
        {
            if (!Program.CurrentSchoolYear.IsClosed)
            {
                ...
                {
                    record.Enrolling = selectedEnrolling;
                    ShowEditSchoolSupplieDiscountForm(record);
                }
        ...
        private void ShowAddSchoolSupplieDiscountForm()
        {
            if (!Program.CurrentSchoolYear.IsClosed)
            {
        ...
        private void ShowEditSchoolSupplieDiscountForm(SchoolSupplieDiscount discount)
        {
            if (!Program.CurrentSchoolYear.IsClosed)
            {
                var form = Program.ServiceProvider.GetService<EditSchoolSupplieDiscountForm>();
