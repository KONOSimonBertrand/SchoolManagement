...
namespace Primary.SchoolApp.Services
{
    public class ListingService
    {
        ...
        public ListingService(IEmployeeService employeeService, ISchoolClassService schoolClassService, IStudentEnrollingService studentEnrollingService, IContactService contactService)
        {
        ...
        public static List<ListingItem> GetListingItems()
        {
            List<ListingItem> items = new()
            {
                ...
                {
                    Id = 12,
                    FrenchName = "RAPPORT GLOBAL DES SALAIRES",
                    EnglishName = "GLOBAL SALARY REPORT",
                    FrenchDescription = "Double-cliquer ici pour consulter les salaires des employés ",
                    EnglishDescription = "Double-click here to view employee salaries ",
                    ModuleId = 10,
                },
                 new()
                {
        ...
        public async Task<Dictionary<int, DataTable>> GetCashFlowList()
        {
            ...
            var monthList = cashFlowList.DistinctBy(x => x.Date.Month).Select(x => x.Date.Month);
            var typeList = Program.CashFlowTypeList;
            DataTable globalDataTable = new();
            DataTable detailDataTable = new();
            ...
            {
                globalDataTable.Columns.Add(AppUtilities.MonthToShortName(m), typeof(string));
            }
